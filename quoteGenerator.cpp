...
void Main(
	...
	vector<vector<string>> subtopics,
	unordered_multimap<string, string> quotesMap,
	vector<string> &prevCustomQuotes,
	vector<string> &prevRandomQuotes);
...
void Main(
	...
	vector<string> &prevRandomQuotes) {
	string input;
	int topicNum, subtopicNum, endNum;
	unordered_map<int, string> subtopicsMap;

	// Get topic from user
	cout << endl
		 << "Select a topic of interest: (Type the numeric value below)"
		 << endl;
	...
	if (topicNum == random) {
		...
		cout << endl
			 << "Program terminated. Click \"Run\" to use the Custom Quote "
				"Generator again.";
		exit(0);
	}
	...
	cout << endl
		 << "Select a subtopic of interest: (Type the numeric value below)"
		 << endl;
	...
	int goBack = subtopics[topicNum].size() + 1;
	end = subtopics[topicNum].size() + 2;

	cout << endl
		 << goBack << ": "
		 << "Return to main screen" << endl
		 << end << ": "
		 << "Exit program" << endl
		 << endl
		 << "Enter the numeric value of your choice: ";
	...
	do {
		cout << endl
			 ...
			 << ((subtopicName == "Artificial Intelligence " ||
				  subtopicName == "Robotics ")
					 ? ""
					 : " ")
			 << "quote" << endl // minor hack since subtopic name appears twice
		...
		case 4:
			cout << endl
				 << "Program terminated. Click \"Run\" to use the Quote "
					"Generator again.";
			exit(0);
			break;
		}
...
void Welcome() {
	string welcomeMessage = "Welcome to the Quote Generator!";
...
void ParseFile(
	...
	vector<vector<string>> &subtopics,
	unordered_multimap<string, string> &quotesMap) {
	string topic = "MAINTOPIC: ", subtopic = "SUBTOPIC: ", quote = "QUOTE: ",
		   endfile = "ENDFILE";
	vector<string> tempSubtopics;
...
void SetQuotes(
	...
	if (line.find(quote) != string::npos) {
		...
		quotesMap.insert(
			make_pair(tempSubtopics[tempSubtopics.size() - 1], tempQuote));
	}
}
...
bool IsInputValid(string input, int limit, int &inputNum) {
	try {
		...
		cout << endl
			 << "Invalid input. Please enter the numeric value "
			 << "(1-" << limit << ")"
...
string FormatQuote(string quote) {
	bool source = false;
	for (unsigned int charIndex = 0, afterNewline = 0;
		 charIndex < quote.length();
		 charIndex++, afterNewline++) {
		if (quote[charIndex] == '\n') {
			afterNewline = 0;
		}
